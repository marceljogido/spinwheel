...
3. Run database migrations:
   ...
   Alternatively, open the SQL editor in pgAdmin and execute the file to create the schema.
...
| POST   | `/prizes`          | Create a new prize entry.                             |
| PUT    | `/prizes/:id`      | Update prize fields such as name, quota, or chances.  |
| DELETE | `/prizes/:id`      | Remove a prize from the catalog.                      |
| POST   | `/prizes/:id/win`  | Increment the `won` counter when a participant wins.  |
| POST   | `/prizes/reorder`  | Persist a new manual ordering for the prize segments. |
